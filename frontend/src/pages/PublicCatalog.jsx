...
export default function PublicCatalog() {
  ...
  return <main className="public-catalog">
    ...
    {emp.tabelasPreco?.length > 0 && <section className="public-section"><h2>Condições comerciais</h2>{emp.tabelasPreco.map(table => <div key={table.id}><h3>{table.nome}</h3>{(table.series || []).map(serie => <p key={serie.id}>{serie.nome} ({serie.quantidade}x): <strong>{money(serie.valor)}</strong></p>)}</div>)}</section>}
